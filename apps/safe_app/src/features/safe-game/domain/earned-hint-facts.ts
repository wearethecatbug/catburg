...
export type EarnedHintFact =
  ...
  | {
      ...
      kind: "divisibility";
      divisor: number;
      relation: "divisible" | "not-divisible";
    }
  | {
      id: string;
      predicateId: string;
...
const RANGE_THRESHOLDS = Array.from(
  { length: 99 },
  (_, index) => (index + 1) * 10,
);
...
function isSafeCode(value: number): boolean {
  return (
    ...
    value >= SAFE_CODE_MINIMUM &&
    value <= SAFE_CODE_MAXIMUM
  );
}
...
function issuedPredicateSet(knowledge: PublicHintKnowledge): Set<string> {
  return new Set([
    ...knowledge.issuedPredicateIds,
    ...knowledge.facts.map(predicateIdFromFact),
  ]);
}
...
function createDivisibilityPredicate(divisor: number): HintPredicate {
  return {
    id: `divisibility:${divisor}`,
    kind: "divisibility",
    divisor,
    matches: (code) => code % divisor === 0,
  };
}

function createRangePredicate(
  threshold: number,
): Extract<HintPredicate, { kind: "range" }> {
  return {
    id: `range:${threshold}`,
    kind: "range",
    threshold,
    matches: (code) => code <= threshold,
  };
}
...
function isEligible(
  ...
  return (
    ...
    nonMatchingCount < candidates.length
  );
}
...
function eligiblePredicateForFamily(
  ...
  if (family === "parity") {
    ...
    return !issued.has(predicate.id) && isEligible(candidates, predicate)
      ? predicate
      : null;
  }
  ...
  if (family === "divisibility") {
    for (const divisor of DIVISORS) {
      ...
      if (!issued.has(predicate.id) && isEligible(candidates, predicate))
        return predicate;
    }
    return null;
  }
  ...
  const eligible: Extract<HintPredicate, { kind: "range" }>[] =
    RANGE_THRESHOLDS.map(createRangePredicate).filter(
      (predicate) =>
        !issued.has(predicate.id) && isEligible(candidates, predicate),
    );
  eligible.sort(
    (left, right) =>
      Math.abs(left.threshold - candidateMedian) -
        Math.abs(right.threshold - candidateMedian) ||
 ...
 */
export function buildPublicCandidateCodes(
  knowledge: Pick<PublicHintKnowledge, "facts" | "wrongAttempts">,
): number[] {
  const wrongAttempts = new Set(knowledge.wrongAttempts.filter(isSafeCode));
  return Array.from(
    { length: SAFE_CODE_MAXIMUM },
    (_, index) => index + SAFE_CODE_MINIMUM,
  )
    .filter((code) => !wrongAttempts.has(code))
    .filter((code) =>
      knowledge.facts.every((fact) => factMatchesCode(fact, code)),
    );
}

export function filterCandidateCodesByFact(
  candidates: readonly number[],
  fact: EarnedHintFact,
): number[] {
...
function factMatchesCode(fact: EarnedHintFact, code: number): boolean {
  if (fact.kind === "parity")
    return fact.parity === "even" ? code % 2 === 0 : code % 2 !== 0;
 ...
 */
export function chooseNextHintPredicate(
  knowledge: PublicHintKnowledge,
): HintPredicate | null {
  ...
  const lastFamily = [
    ...
    .at(-1);
  const startIndex = lastFamily
    ? (FAMILY_ORDER.indexOf(lastFamily) + 1) % FAMILY_ORDER.length
    : 0;
...
/**
 * Evaluates the private code only after the reducer accepts a correct active challenge; range facts
 * stay within the current public bounds.
 */
export function evaluateEarnedHintFact(
  predicate: HintPredicate,
  code: number,
  ...
  if (predicate.kind === "parity")
    return {
      ...
      kind: "parity",
      parity: predicate.matches(code) ? "even" : "odd",
    };
  if (predicate.kind === "divisibility")
    return {
      ...
      relation: predicate.matches(code) ? "divisible" : "not-divisible",
    };
  ...
  return predicate.matches(code)
    ? {
        ...
        kind: "range",
        minimum,
        maximum: Math.min(maximum, predicate.threshold),
      }
    : {
        id,
        predicateId: predicate.id,
        kind: "range",
        minimum: Math.max(minimum, predicate.threshold + 1),
        maximum,
      };
}
