...
function scrollStoredHints(event: KeyboardEvent<HTMLUListElement>) {
  ...
  const rowStep = 28;
  const nextTop = event.key === "End" ? list.scrollHeight
    : event.key === "Home" ? 0
      : event.key === "PageDown" ? list.scrollTop + list.clientHeight
        : event.key === "PageUp" ? list.scrollTop - list.clientHeight
          : event.key === "ArrowDown" ? list.scrollTop + rowStep
            : event.key === "ArrowUp" ? list.scrollTop - rowStep
              : null;
...
export function StoredHintsCard({ facts, onPointerEnter, onPointerLeave }: { facts: readonly EarnedHintFact[]; onPointerEnter?: () => void; onPointerLeave?: () => void }) {
 return <section id="stored-hints-card" className={styles.card} aria-label="Stored hints" onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave}><h2>Stored hints</h2><ul aria-label="Stored hint list" tabIndex={0} onKeyDown={scrollStoredHints}>{facts.map((fact) => <li key={fact.id}>{factText(fact)}</li>)}</ul></section>;
}
