...
import { selectEarnedHintStatusText, selectTerminalPresentation } from "../model/game.selectors";
...
function SafeGameScreenContent() {
  ...
  const earnedHintStatus = selectEarnedHintStatusText(state);
  ...
  return (
    <main className={styles.safeGameScreen}>
      ...
      <div className={styles.codeEntry}>
        ...
        {earnedHintStatus && <p className={styles.earnedHint} role="status">{earnedHintStatus}</p>}
      </div>
