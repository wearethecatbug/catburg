...
import type { EarnedHintFact, SafeGameState, SafeHint } from "../domain";

/** Domain terminal facts only; presentation art belongs to presentation-machine. */
export function selectTerminalPresentation(state: SafeGameState): {
  safeOpen: boolean;
  revealedCode: number | null;
} {
  if (state.phase === "won") return { safeOpen: true, revealedCode: null };
