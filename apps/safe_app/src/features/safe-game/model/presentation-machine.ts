...
export function reducePresentation(state: PresentationState | null, event: PresentationEvent, now: number): PresentationState {
  ...
  if (e.type === "HISTORY_TOGGLE") { if (s.mode === "HISTORY") return s.resumeMode === "HINT_REWARD" ? restoreHistoryReward(s, now) : resumeHistory(s, now, s.resumeMode); return s.mode === "HINT_REWARD" ? historyReward(s, now) : history(s, now, s.mode === "STORED_HINTS" ? s.resumeMode : activeBase(s)); }
