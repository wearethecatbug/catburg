...
export type PresentationState = Common & (
  ...
  | ({ mode: "HISTORY"; surface: "history"; resumeMode: "HINT_REWARD"; rewardResumeMode: ActiveBase; challengeId: string; award: PublicAward } & Clock<"reward-expiry">)
...
function validateState(value: unknown): PresentationState {
  ...
  if (m === "HISTORY" && raw.resumeMode === "HINT_REWARD") { if (raw.rewardResumeMode !== "PLAYING" && raw.rewardResumeMode !== "WRONG") fail(); const id = string(raw.challengeId); if (!challengeIds.includes(id)) fail(); const current = award(raw.award); const latest = publicAwards.at(-1); if (!latest || current.factId !== latest.factId || current.text !== latest.text) fail(); }
...
function historyReward(state: Extract<PresentationState, { mode: "HINT_REWARD" }>, now: number): PresentationState { if (!state.timer) fail(); const next = base(state, now, { idleStartedAt: now, promptAnchorAt: now }); return { ...next, mode: "HISTORY", surface: "history", resumeMode: "HINT_REWARD", rewardResumeMode: state.resumeMode, challengeId: state.challengeId, award: state.award, ...clock(next, now, "reward-expiry", state.timer.dueAt) } as PresentationState; }
function restoreHistoryReward(state: Extract<PresentationState, { mode: "HISTORY"; resumeMode: "HINT_REWARD" }>, now: number): PresentationState { if (!state.timer) fail(); if (now >= state.timer.dueAt) return toBase(state, now, state.rewardResumeMode); const next = base(state, now); return { ...next, mode: "HINT_REWARD", surface: "hint-reward", resumeMode: state.rewardResumeMode, challengeId: state.challengeId, award: state.award, ...clock(next, now, "reward-expiry", state.timer.dueAt) } as PresentationState; }
...
export function reducePresentation(state: PresentationState | null, event: PresentationEvent, now: number): PresentationState {
  ...
  if (e.type === "INPUT_VALUE_CHANGED") { if (e.inputRevision <= s.inputRevision || s.mode === "HINT_DIALOG" || s.mode === "HINT_SUCCESS_HANDOFF" || s.mode === "HINT_REWARD" || (s.mode === "HISTORY" && s.resumeMode === "HINT_REWARD")) return s; if (s.mode === "HISTORY") return history({ ...s, inputRevision: e.inputRevision } as PresentationState, now, "PLAYING"); return playing({ ...s, inputRevision: e.inputRevision } as PresentationState, now); }
  if (e.type === "HISTORY_TOGGLE") { if (s.mode === "HISTORY") return s.resumeMode === "HINT_REWARD" ? restoreHistoryReward(s, now) : resumeHistory(s, now, s.resumeMode); return s.mode === "HINT_REWARD" ? historyReward(s, now) : history(s, now, s.mode === "STORED_HINTS" || s.mode === "TERMINAL_STORED_HINTS_PENDING" ? s.resumeMode : activeBase(s)); }
  ...
  if ("token" in e) {
    ...
    if (kind === "reward-expiry") {
      if (s.mode === "HISTORY" && s.resumeMode === "HINT_REWARD") return history(s, now, s.rewardResumeMode);
