...
import { Dropdown, DropdownItem, SortIcon, ChevronDownIcon } from '@/components';
        ...
        <span className="flex items-center gap-1 px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
          ...
          <ChevronDownIcon
            ...
            className={`transition-transform ${
              state.sort.direction === 'asc' ? 'rotate-180' : ''
            }`}
          />
        </span>
      }
        ...
        >
          <span className="flex items-center justify-between w-full">
            ...
            {state.sort.field === option.field && (
              <ChevronDownIcon
                size="sm"
                className={state.sort.direction === 'asc' ? 'rotate-180' : ''}
              />
            )}
          </span>
        </DropdownItem>
